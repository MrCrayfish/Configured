package com.mrcrayfish.configured.fabric.integration;
...
public final class CatalogueConfigFactory
{
    public static Screen createConfigScreen(Screen currentScreen, ModContainer container)
    {
        ...
        Set<IModConfig> configs = ClientHandler.getProviders().stream().flatMap(provider -> provider.getConfigurationsForMod(Platform.getMod(container.getMetadata().getId())).stream()).collect(Collectors.toSet());
