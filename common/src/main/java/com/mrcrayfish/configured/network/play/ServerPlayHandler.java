...
public class ServerPlayHandler
{
    ...
    public static void handleSyncSimpleConfigMessage(Connection connection, ServerPlayer player, ResourceLocation id, byte[] data)
    {
        ...
        if(SimpleConfigManager.getInstance().processSyncData(id, data, true))
        {
            ...
            {
                Collection<ServerPlayer> p = player.server.getPlayerList() != null ? Collections.unmodifiableCollection(player.server.getPlayerList().getPlayers()) : Collections.emptyList();
                p.forEach(player1 -> {
                    NetworkManager.sendToPlayer(player1, MessageSyncSimpleConfig.ID, MessageSyncSimpleConfig.create(id, data));
                });
            }
    ...
    public static void handleRequestSimpleConfigMessage(ServerPlayer player, ResourceLocation id)
    {
        ...
        try
        {
            ...
            NetworkManager.sendToPlayer(player, MessageResponseSimpleConfig.ID, MessageResponseSimpleConfig.create(key, data));
        ...
        catch(Exception e)
        {
            ...
            NetworkManager.sendToPlayer(player, MessageResponseSimpleConfig.ID, MessageResponseSimpleConfig.create(config.getName(), new byte[]{}));
        }
    }
